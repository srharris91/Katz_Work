...
{
  // set flagG to the node number of the element verticies
  Array1D<int> flagG(nNodeG);
  flagG.set(-1);
  nNode = 0;
  for (int n=0; n<nTriG; n++)
    for (int k=1; k<4; k++)
      if (flagG(triG[n][k]) == -1) flagG(triG[n][k]) = nNode++;
  ...
  */
  ...
  for (int n=0; n<nElem; n++){
    m = 0;
    //corner verticies
    for (int j=1; j<4; j++) elem(n,m++) = flagG(triG[n][j]);
