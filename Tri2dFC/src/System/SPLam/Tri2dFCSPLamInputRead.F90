...
!! \par Source code:
!! \include src/System/SPLam/Tri2dFCSPLamInputRead.F90


SUBROUTINE tri2dfrsplaminputread(&
     fileLen, &
     fileName, &
     nq, &
     nComp, &
...
DO n=1,nBPatches
   ...
   ELSE IF (TRIM(bType(n)) == 'frozen'      ) THEN
      bType0(n) = 7
   ELSE IF (TRIM(bType(n)) == 'symmetry'    ) THEN
      bType0(n) = 8
   ELSE IF (TRIM(bType(n)) == 'periodic'    ) THEN
      bType0(n) = 9
   ELSE IF (TRIM(bType(n)) == 'nothing'     ) THEN
      bType0(n) = 10
   ELSE
...
END SUBROUTINE tri2dfrsplaminputread
