...
INTEGER :: n,m,k,i,j,iu,idim,jdim,wake
REAL :: pi,angle,dx,fact,dx0,dx0a,dx1,dx1a,rmax,x0,x1
REAL,ALLOCATABLE,DIMENSION(:,:) :: x,xp,yp,nbl
CHARACTER(80) :: platefile,fileName,arg


WRITE(*,*)'name of plate file:'
READ(*,*)platefile
WRITE(*,*)'number of points on the airfoil surface'
READ(*,*)nSurfFaces
WRITE(*,*)'output file name:'
READ(*,*)filename

iu = 9
OPEN(UNIT=iu,FILE=platefile,FORM='formatted')
READ(iu,*)
READ(iu,*)idim,jdim
...
! Find which nodes belong to the wake 
wake = (idim - nSurfFaces)/2 

! We have to remove a node later so lets do this now to remove confusion
! with allocate statements
nSurfFaces = nSurfFaces-1


! preliminaries
nMesh         = 1
nSurfNodes    = nSurfFaces 
!nSurfFaces    = idim
nBndEdges     = 0!2
nPtsPerStrand = 0
nSurfPatches  = 1
...
DO i = wake+1,idim-wake-1

   !Remove one of the trailing edge nodes and set the other y value to 0
   IF (i == wake+1) THEN
   xSurf(1,i-(wake)) = xp(i,1)
   xsurf(2,i-(wake)) = 0.00000
   ELSE
   xSurf(1,i-(wake)) = xp(i,1)
   xSurf(2,i-(wake)) = yp(i,1)
   ENDIF
...
print*,
print*,'coordinates of nodes on surface'
do i = 1,nSurfFaces
print*,i,xSurf(1,i),xSurf(2,i)
end do

! form connectivity
DO i=1,nSurfFaces
   
   ! Need to go back to the first node to make airfoil a loop
   IF (i == nSurfFaces) THEN
   surfFaces(1,i) = nSurfFaces
   surfFaces(2,i) = 1
   faceTag(i) = 1
...
! show the connected surfaces
print*,
print*,'connected surfaces'
DO i = 1,nSurfFaces
print*,surfFaces(1,i),surfFaces(2,i)
END DO
