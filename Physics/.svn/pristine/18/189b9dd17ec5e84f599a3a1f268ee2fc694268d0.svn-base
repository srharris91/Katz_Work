...
// [getDensityZ]
void IdealGas::getDensityZ(const int& npts,
			   const double* p,
			   const double* t,
			   const double* pz,
			   const double* tz,
...
{
  for (int n=0; n<npts; n++)
    rhoz[n] = 1./(rGas*t[n])*(pz[n]-p[n]/t[n]*tz[n]);
}
// [getDensityZ]
...
// [getEnthalpyZ]
void IdealGas::getEnthalpyZ(const int& npts,
			    const double* p,
			    const double* t,
			    const double* pz,
			    const double* tz,
...
{
  double gmg = rGas*gamma/(gamma-1.);
  for (int n=0; n<npts; n++) hz[n] = gmg*tz[n];
}
// [getEnthalpyZ]
