  ...
  for (int n=0; n<npts; n++){
    ...
    for (int k=0; k<nq; k++){
      q[iq+k] = rValue[k]*(1.+ax [k]*sin(bx [k]-cx [k]*x)
			     ...
			     +az [k]*sin(bz [k]-cz [k]*z)
			     +axy[k]*sin(bxy[k]-cxy[k]*x*y)
			     +axz[k]*sin(bxz[k]-cxz[k]*x*z)
			     +ayz[k]*sin(byz[k]-cyz[k]*y*z));
    }}
}
// [getQ]
  ...
  for (int n=0; n<npts; n++){
    ...
    for (int k=0; k<nq; k++)
      qx[iq+k] =-rValue[k]*(ax [k]*cx [k]  *cos(bx [k]-cx [k]*x)
			   ...
			   +axz[k]*cxz[k]*z*cos(bxz[k]-cxz[k]*x*z));
  }
}
// [getQx]
  ...
  for (int n=0; n<npts; n++){
    ...
    for (int k=0; k<nq; k++)
      qy[iq+k] =-rValue[k]*(ay [k]*cy [k]  *cos(by [k]-cy [k]*y)
			   ...
			   +ayz[k]*cyz[k]*z*cos(byz[k]-cyz[k]*y*z));
  }
}
// [getQy]
		  ...
		  double* qz)
{
  ...
  for (int n=0; n<npts; n++){
    ...
    z      = xy[ix+2];
    for (int k=0; k<nq; k++)
      qz[iq+k] =-rValue[k]*(az [k]*cz [k]  *cos(bz [k]-cz [k]*z)
			   +axz[k]*cxz[k]*x*cos(bxz[k]-cxz[k]*x*z)
			   +ayz[k]*cyz[k]*y*cos(byz[k]-cyz[k]*y*z));
  }
}
// [getQz]
  ...
  for (int n=0; n<npts; n++){
    ...
    for (int k=0; k<nq; k++)
      qxx[iq+k] =-rValue[k]*(ax [k]*cx [k]*cx [k]    *sin(bx [k]-cx [k]*x)
			    ...
			    +axz[k]*cxz[k]*cxz[k]*z*z*sin(bxz[k]-cxz[k]*x*z));
