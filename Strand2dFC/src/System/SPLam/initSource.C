...
void Strand2dFCSPLam::initSource(const int& npts,
			      const double* x,
			      double* s)
{
  if (isolution == 3){ //mms
	...
	*/
	...
	/*
	syxy     = muy[n]*vx[n]+mu[n]*vxy[n];
	syyy     =-(muy[n]*ux[n]+mu[n]*uxy[n])*2./3.;
	qqyy     = 0.;

	syxy     = muy[n]*uy[n]+mu[n]*uyy[n];
	syyy     =(muy[n]*vy[n]+mu[n]*vyy[n])*4./3.;
	qqyy     =-(ky[n]*ty[n]+k[n]*tyy[n]);
	*/
