				  ...
				  const double* r)
{
  ...
  // entropy
	  ...
	  //~ cout<<"npts "<<npts<<endl;//shaun change for limiter check
    for (int n=0; n<npts; n++){
      iq  = n*nq;
      iqa = n*nqa;
      ffile << fabs(qa[iqa]/pow(q[iq],gamma)-s0) << endl;
      //~ if (fabs(qa[iqa]/pow(q[iq],gamma)-s0)>=10.0) {
		  //~ cout.precision(16);
      //~ cout << fabs(qa[iqa]/pow(q[iq],gamma)-s0) << endl;
      //~ cout<<"qa [iqa] "<<qa[iqa]<<" "<<iqa<<endl;
      //~ cout<<"q [iq] "<<q[iq]<<" "<<iqa<<endl;
      //~ cout<<"gamma "<<gamma<<endl;
      //~ cout<<"s0 "<<s0<<endl;} // shaun addition to see entropy values
    }
  }
