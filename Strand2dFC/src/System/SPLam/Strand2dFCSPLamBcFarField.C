...
Strand2dFCSPLamBcFarField::Strand2dFCSPLamBcFarField()
{
}
// [Strand2dFCSPLamBcFarField]
...
// [BCPenalty]
void Strand2dFCSPLamBcFarField::BCPenalty(const int& inout,
					  const double* A,
					  const double& Pinv0,
					  const double* q,
					  const double* qa,
					  ...
					  double* rb)
{
  ...
  R[4 ] = l[0]*u;
  R[5 ] = l[1]*Tx;
  R[6 ] = l[2]*(u+Nx*c)+a*(u-Nx*c);
  R[7 ] = l[3]*(u-Nx*c)+b*(u+Nx*c);
  
  R[8 ] = l[0]*v;
  R[9 ] = l[1]*Ty;
  R[10] = l[2]*(v+Ny*c)+a*(v-Ny*c);
  R[11] = l[3]*(v-Ny*c)+b*(v+Ny*c);
  
  R[12] = l[0]*qq;
  R[13] = l[1]*ut;
  R[14] = l[2]*(h+un*c)+a*(h-un*c);
  R[15] = l[3]*(h-un*c)+b*(h+un*c);
...
// [BCPenaltyVis]
void Strand2dFCSPLamBcFarField::BCPenaltyVis(const double& Pinv0,
					     const double* q,
					     const double* qa,
					     const double* gv,
					     const double* uw,
...
// [BCPenaltyJacobian]
void Strand2dFCSPLamBcFarField::BCPenaltyJacobian(const int& inout,
						  const double* A,
						  const double& Pinv0,
						  const double* q,
						  const double* qa,
