...
void Strand2dFCBlockSolver::fdCoeffViscous()
{
  if (strandOrder == 1 || strandOrder == 2){ // D_2^(2,1,2)
    ...
    for (int j=0; j<nStrandNode; j++) vcn2[j] = new int[3];
    ...
    for (int j=0; j<nStrandNode; j++){
      nj      = vcn2[j][1];
      vcn3[j] = new int*[nj];
      for (int m=0; m<nj; m++) vcn3[j][m] = new int[3];
    }
	...
	vcn3[j][m][1] = nl;
	vcn3[j][m][2] = j-nStrandNode+nBnd+1;
      }
      i--;
    }
	...
	vcn3[j][m][0] = nStrandNode-(l1+nl);
	vcn3[j][m][1] = nl;
	vcn3[j][m][2] = j-nStrandNode+nBnd+2;
      }
      i--;
    }
	...
	vcn3[j][m][0] = nStrandNode-(l1+nl);
	vcn3[j][m][1] = nl;
	vcn3[j][m][2] = j-nStrandNode+nBnd+3;
      }
      i--;
    }
