...
//shaun change output surface pressure
//shaun addition for limiter output to check pressure
	ofstream limfile;
	a << "pressure.dat";
	if (step == 0) limfile.open (a.str().c_str());
    else limfile.open (a.str().c_str(),ios::app);
    limfile.setf(ios::scientific);
    limfile.precision(14);
    a.str("");
    a.clear();
    //shaun change output surface pressure to file
    for(int n=0;n<nSurfNode;n++){
		limfile << surfX(n,0) << " " << surfX(n,1) << " " << qa(n,0,0) <<" " << -(qa(n,0,0)-qa(0,0,0))/(.5*q(0,0,0)*(qa(0,0,1)*qa(0,0,1)+qa(0,0,2)*qa(0,0,2))) <<endl;
	}
	limfile.close();
  ...
  if (iSurfFile != 0){
	...
	nj = icn2[0][1];
	for (int m=0; m<nj; m++){
	  for (int k=0; k<nq ; k++) qnN (i,k) += dnr*icn1[0][m]*q (ni,m,k);
	  for (int k=0; k<nqa; k++) qanN(i,k) += dnr*icn1[0][m]*qa(ni,m,k);
	  j1++;
	}}
