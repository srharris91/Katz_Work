...
void Strand2dFCBlockSolver::mapping()
{
  ...
  int n=0; //note: choose n=0 and then copy to all strands which are identical
  for (int j=0; j<nIcbNode; j++) //root
    for (int m=0; m<nIcb; m++) xn(n,j) += icb(j,m)*strandX(m);
  for (int j=nIcbNode; j<nStrandNode-nIcbNode; j++) //interior
    for (int m=0; m<nIci; m++)
      xn(n,j) += ici(m)*(strandX(j+m+1)-strandX(j-m-1));
  jH = nIcbNode-1;
  for (int j=nStrandNode-nIcbNode; j<nStrandNode; j++){ //tip
    mH = nIcb-1;
    for (int m=0; m<nIcb; m++){
      xn(n,j) -= icb(jH,mH)*strandX(nStrandNode-nIcb+m);
      mH--;
    }
    jH--;
  }
